...
addClient u con ip new = do
  ...
  verifyCode (newClientVerificationCode new) Code.Login (userId . accountUser $ acc)
  ...
  where
    ...
    verifyCode :: Maybe Code.Value -> Code.VerificationAction -> UserId -> ExceptT ClientError (AppIO r) ()
    verifyCode mbCode action userId =
      UserAuth.verifyCode mbCode action userId `catchE` \case
        VerificationCodeRequired -> error "todo"
        VerificationCodeNoPendingCode -> error "todo"
        VerificationCodeNoEmail -> error "todo"
